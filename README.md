...
注意：导入名是 `sciencedb`，不是仓库名/发布名 `funapi-sciencedb`（见 [NAMING.md](https://github.com/farfarfun/todo-list/blob/master/NAMING.md)）。
...
```python
from sciencedb import Client
from sciencedb.api.open_api_controller import search_using_get
...
由于是自动生成的客户端，具体字段和返回结构以 `src/sciencedb/models/` 下的类定义为准。
